...
static struct argp_option options[] =
{
    ...
    {"gray",         'e', "FILE", 0, "Turn image colors to gray scalar"},
    { 0 }
};

struct arguments {
    int rotate_left, rotate_rigth, mirror_hor, mirror_ver, gray_scalar;
    char *output_file;
    char *input_file;
};
...
void exec_options(struct arguments args, Image image, pixel **buffer) {
    ...
    } else if (args.gray_scalar) {
        gray_scalar(image, buffer);
    }
}
